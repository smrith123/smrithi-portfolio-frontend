 ...
 *
 * Pages stay statically rendered and refresh themselves every 10 seconds, so an
 * admin edit is live almost immediately without making every visit hit the API.
 *
 * ponytail: time-based revalidation. Swap for an on-demand revalidate webhook
 * from the admin if instant updates ever matter more than this simplicity.
 */
